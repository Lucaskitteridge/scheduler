  ...
  days: [
    {
      ...
      name: "Monday",
      appointments: [1, 2, 3],
      interviewers: ["Lucas Kitteridge", "Sophie Ryder", "John Stamos"],
    },
    {
      id: 2,
      name: "Tuesday",
      appointments: [4, 5],
      interviewers: ["Morley Willoughby", "Nicole Kuzmitch"],
    },
  ],
  appointments: {
...
test("getInterviewers for day returns an array", () => {
  const result = getInterviewersForDay(state, "Monday");
  expect(Array.isArray(result)).toBe(true);
});

test("getInterviewers returns an array with a length matching the number of interviews for that day", () => {
  const result = getInterviewersForDay(state, "Monday");
  expect(result.length).toEqual(3);
});
...
test("getInterviewers returns an array containing the correct interviewer objects", () => {
  ...
  expect(first).toEqual(state.interviewers["Morley Willoughby"]);
  expect(second).toEqual(state.interviewers["Nicole Kuzmitch"]);
});
...
test("getInterviewers returns an empty array when the day is not found", () => {
  const result = getInterviewersForDay(state, "Murnday");
  expect(result.length).toEqual(0);
});
